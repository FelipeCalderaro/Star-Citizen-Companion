...
class HotkeysService(BaseService):
    ...
    def register_hotkeys(self):
        ...
        self.register_hotkey("alt+=", lambda: asyncio.run(self._send_event("togglevisibility")))
        self.register_hotkey("alt+shift+=", lambda: asyncio.run(self._send_event("togglemovement")))
    ...
    def start_listening_keys(self):
        ...
        keyboard.on_press_key("f", lambda _: asyncio.run(self._send_event("f_down")))
        keyboard.on_release_key("f", lambda _: asyncio.run(self._send_event("f_up")))
